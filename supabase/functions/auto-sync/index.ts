...
async function mapearPartida(event: any): Promise<number | null> {
  ...
  if (match) {
    await supabase.from('partidas').update({ api_event_id: event.id }).eq('id', match.id)
    return match.id
  }
...
Deno.serve(async (_req) => {
  try {
    ...
    // 1. Verificar orçamento diário
    const { data: reqHoje } = await supabase.rpc('fn_requisicoes_hoje')
    const usadas = reqHoje ?? 0
    if (usadas >= LIMITE_DIARIO) {
      return respond({ skipped: true, reason: `Limite diário atingido (${usadas}/${LIMITE_DIARIO})` })
    }

    // 2. Verificar janela de jogos
    const { data: janela } = await supabase.rpc('fn_janela_jogos')
    if (janela === 'fora') {
      return respond({ skipped: true, reason: 'Fora da janela de jogos' })
    }

    // 3. Determinar quais rodadas buscar
    const { data: rodadasAtivas } = await supabase.rpc('fn_rodadas_ativas_hoje')

    // Extrair números de rodada das strings (ex: "Group A - 1" → 1)
    const roundNumbers = new Set<number>()
    if (rodadasAtivas && Array.isArray(rodadasAtivas)) {
      for (const r of rodadasAtivas) {
        const match = String(r).match(/(\d+)/)
        if (match) roundNumbers.add(parseInt(match[1]))
      }
    }

    // Se não encontrou rodadas específicas, usar rodadas 1-3
    const rounds = roundNumbers.size > 0 ? [...roundNumbers] : [1, 2, 3]

    // Verificar se temos orçamento para todas as rodadas
    const restante = LIMITE_DIARIO - usadas
    const roundsParaBuscar = rounds.slice(0, restante)

    if (roundsParaBuscar.length === 0) {
      return respond({ skipped: true, reason: 'Sem orçamento para mais rodadas' })
    }
    ...
    let totalEventos = 0
    const requisicoesApi = roundsParaBuscar.length

    for (const round of roundsParaBuscar) {
    ...
    // 5. Backup automático de rodada completa
        ...
        for (const [rodada, info] of Object.entries(porRodada)) {
          if (info.total > 0 && info.total === info.encerradas) {
            const { data: jaExiste } = await supabase
              .from('backups_rodada').select('id').eq('rodada', rodada).eq('tipo', 'auto').single()
            ...
            if (!jaExiste) {
              ...
              await supabase.from('backups_rodada').insert({
                rodada, tipo: 'auto',
                partidas: partidasRod || [], palpites: palpitesRod || [], ranking,
    ...
    await supabase.from('sync_log').insert({
      tipo: 'auto',
      rodadas_consultadas: roundsParaBuscar,
      requisicoes_api: requisicoesApi,
      eventos_processados: totalEventos,
      atualizados,
      pontos_recalculados: pontosRecalculados,
      backups_gerados: backupsGerados,
    })

    return respond({
      ok: true,
      janela,
      requisicoesHoje: usadas + requisicoesApi,
      limiteHoje: LIMITE_DIARIO,
      rodadas: roundsParaBuscar,
      totalEventos,
      atualizados,
      pontosRecalculados,
      backupsGerados,
