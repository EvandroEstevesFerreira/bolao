...
function AbaUsuarios() {
  ...
  async function cadastrarUsuario(e) {
    ...
    if (error) {
      setErro(error.message.includes('duplicate') ? 'CPF, e-mail ou telefone já cadastrado.' : error.message)
    } else {
        ...
        {carregando ? (
          ...
          <div className="space-y-2">
            {usuariosFiltrados.map(u => (
              <div
                key={u.id}
                className="border border-gray-100 dark:border-[#2A3942] rounded-xl overflow-hidden"
              >
                {/* Linha resumo */}
                <div
                  onClick={() => setExpandido(expandido === u.id ? null : u.id)}
                  className="flex items-center gap-3 px-3 py-2.5 cursor-pointer hover:bg-gray-50 dark:hover:bg-[#2A3942]/50 transition-colors"
                >
                  <div className="w-9 h-9 rounded-full bg-gradient-to-br from-primary/20 to-[#25D366]/20 flex items-center justify-center flex-shrink-0">
                    <span className="text-xs font-bold text-primary">
                      {(u.nickname || u.nome || '??').slice(0, 2).toUpperCase()}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    ...
                    {u.is_admin && (
                      <span className="px-1.5 py-0.5 bg-teal-100 dark:bg-teal-500/10 text-teal-700 dark:text-teal-400 rounded text-[10px] font-bold">ADM</span>
                    )}
                ...
                {expandido === u.id && (
                  <div className="px-3 pb-3 border-t border-gray-100 dark:border-[#2A3942] bg-gray-50/50 dark:bg-[#1A2329]/50">
                    <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm mt-2.5">
                      <div>
                        <dt className="text-[10px] text-gray-400 uppercase tracking-wider">Nome</dt>
                        <dd className="font-medium text-xs">{u.nome}</dd>
                      </div>
                      <div>
                        <dt className="text-[10px] text-gray-400 uppercase tracking-wider">Nickname</dt>
                        <dd className="font-medium text-xs">@{u.nickname || '—'}</dd>
                      </div>
                      <div>
                        <dt className="text-[10px] text-gray-400 uppercase tracking-wider">CPF</dt>
                        <dd className="font-medium text-xs font-mono">{formatarCpfExibicao(u.cpf)}</dd>
                      </div>
                      <div>
                        <dt className="text-[10px] text-gray-400 uppercase tracking-wider">E-mail</dt>
                        <dd className="font-medium text-xs truncate">{u.email || '—'}</dd>
                      </div>
                      <div>
                        <dt className="text-[10px] text-gray-400 uppercase tracking-wider">Telefone</dt>
                        <dd className="font-medium text-xs">{formatarTelExibicao(u.telefone)}</dd>
                      </div>
                      <div>
                        <dt className="text-[10px] text-gray-400 uppercase tracking-wider">Setor/CR</dt>
                        <dd className="font-medium text-xs">{u.setor_cr || '—'}</dd>
                      </div>
                      <div>
                        <dt className="text-[10px] text-gray-400 uppercase tracking-wider">Cadastro</dt>
                        <dd className="font-medium text-xs">{u.criado_em ? new Date(u.criado_em).toLocaleDateString('pt-BR') : '—'}</dd>
                      </div>
                      <div>
                        <dt className="text-[10px] text-gray-400 uppercase tracking-wider">Auth</dt>
                    ...
                    <div className="flex gap-2 mt-3">
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${
                          u.ativo
                            ...
                            : 'bg-green-50 dark:bg-green-500/10 text-green-600 hover:bg-green-100'
                        }`}
                      >
                      ...
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${
                          u.is_admin
                            ? 'bg-gray-100 dark:bg-[#2A3942] text-gray-600 dark:text-[#8696A0] hover:bg-gray-200'
                            : 'bg-teal-50 dark:bg-teal-500/10 text-teal-600 hover:bg-teal-100'
                        }`}
                      >
